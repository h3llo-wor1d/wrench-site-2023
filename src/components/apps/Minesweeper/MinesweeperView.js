...
function MineSweeperView({
  ...
  return (
    <div className={className} onContextMenu={e => e.preventDefault()}>
      ...
      <section className="mine__content" style={{}} onMouseDown={onMouseDownContent}>
        <div className="mine__score-bar">
          <div className="mine__digits__outer">
            {renderDigits(remainMines())}
          </div>
          <div className="mine__face__outer">
            <button ref={face} className="mine__face" onClick={() => onReset()} style={{position: "absolute", left: "50%", transform: "translateX(-50%)"}}>
              {statusFace()}
...
export default styled(MineSweeperView)`
  ...
  .mine__content__inner {
    display: grid;
    grid-template-columns: repeat(${({ columns }) => columns}, 23.62222222px);
    grid-template-rows: repeat(${({ rows }) => rows}, 23.62222222px);
